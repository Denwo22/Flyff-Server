...
#endif // __PCBANG
{
	...
	ON_MSG( PACKETTYPE_SAVE_SKILL, &CDPTrans::OnSaveSkill );
#endif // __S_NEW_SKILL_2
	ON_MSG( PACKETTYPE_LOG_PLAY_CONNECT, &CDPTrans::OnLogPlayConnect );
	ON_MSG( PACKETTYPE_PREVENT_LOGIN, &CDPTrans::OnPreventLogin );
	ON_MSG( PACKETTYPE_LOG_PLAY_DEATH, &CDPTrans::OnLogPlayDeath );
	ON_MSG( PACKETTYPE_LOG_LEVELUP, &CDPTrans::OnLogLevelUp );
	ON_MSG( PACKETTYPE_LOG_SERVER_DEATH, &CDPTrans::OnLogServerDeath );
	...
	ON_MSG( PACKETTYPE_LOG_GAMEMASTER_CHAT, &CDPTrans::OnGammaChat );
	ON_MSG( PACKETTYPE_LOG_PK_PVP, &CDPTrans::OnLogPkPvp );
	ON_MSG( PACKETTYPE_LOG_SCHOOL, &CDPTrans::OnLogSchool );
	ON_MSG( PACKETTYPE_SKILLPOINTLOG, &CDPTrans::OnLogSkillPoint );
	...
	ON_MSG( PACKETTYPE_GUILD_DB_REALPENYA, &CDPTrans::OnGuildRealPay );
	ON_MSG( PACKETTYPE_WD_GUILD_CONTRIBUTION, &CDPTrans::OnGuildContribution );

	ON_MSG( PACKETTYPE_UPDATE_GUILD_RANKING, &CDPTrans::OnUpdateGuildRanking );
	ON_MSG( PACKETTYPE_UPDATE_GUILD_RANKING_DB, &CDPTrans::OnUpdateGuildRankingDB );

	ON_MSG( PACKETTYPE_SCHOOL_REPORT, &CDPTrans::OnSchoolReport );
	ON_MSG( PACKETTYPE_BUYING_INFO, &CDPTrans::OnBuyingInfo );
	...
	ON_MSG( PACKETTYPE_GETPENYAGUILD_GUILDCOMBAT, &CDPTrans::OnGCGetPenyaGuild );
	ON_MSG( PACKETTYPE_GETPENYAPLAYER_GUILDCOMBAT, &CDPTrans::OnGCGetPenyaPlayer );
...
#ifdef __VTN_TIMELIMIT
	ON_MSG( PACKETTYPE_TIMELIMIT_INFO, &CDPTrans::OnTimeLimitInfoReq );
	...
	ON_MSG( PACKETTYPE_QUIZ_STATE, &CDPTrans::OnQuizEventState );
	ON_MSG( PACKETTYPE_QUIZ_ENTRANCE, &CDPTrans::OnQuizEventEntrance );
	ON_MSG( PACKETTYPE_QUIZ_SELECT, &CDPTrans::OnQuizEventSelect );
	ON_MSG( PACKETTYPE_QUIZ_PRIZEITEM, &CDPTrans::OnPostPrizeItem );
#endif // __QUIZ
