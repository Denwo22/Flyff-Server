...
void CWndMapEx::OnLButtonDown( UINT nFlags, CPoint point )
{
	...
	else
	{
		...
		if( g_WndMng.m_pWndWorld->m_bCtrlPushed == TRUE )
		{
			...
			prj.m_MapInformationManager.InsertUserMarkPositionInfo( _T( "위치" ), vReconvertedPosition.x, vReconvertedPosition.z );
			return;
		}
...
void CWndMapEx::ProcessMonsterInformationToolTip( void )
{
	...
	CWndButton* pWndButton = ( CWndButton* )GetDlgItem( WIDC_CHECK_MONSTER_INFORMATION );
	if( pWndButton == NULL )
	{
		return;
	}
