...
*/
	...
	//------------- 공격중이 아님. --------------------
	{
		...
		if( timeGetTime() >= m_tmReattack + (2000 - m_tmAddReattack) )		// 리어택 시간이 됐는가?  기본적으로 5초간격에 AddReattack으로 줄어들 수 있음.
		{
			...
			{
//				Error( "CAIBigMuscle::MoveProcessRage : 타겟이 없다" );
				return FALSE;
			}
			...
			if( fDistSq < fRadiusSq * 4.0f )		// 근거리면.
			{
				...
				if( dwNum < 85 )
				{
					...
					fArrivalRange = 5.0f;				// 딱붙어서 밟아야 한다.
				}
				else
				{
			...
			{								
				...
				{						
					...
					fArrivalRange = 15.0f;			// xx미터 까지 접근하자.
				}
				else
				{
					m_idTarget = NULL_ID;
...
#ifdef __INTERNALSERVER
			{
				...
				case CAT_QUAKE_ONE:		strType = "두손내려침-광역";	break;
				case CAT_QUAKEDOUBLE:		strType = "두손내려침";	break;
				case CAT_NORMAL2:		strType = "한손 내려침";	break;
				}
				CString str;
